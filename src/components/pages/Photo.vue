<template>
    ...
    <div>
        <pagination @paginate="paginate($event)" v-model="page" :records="total" :per-page="per_page" :options="options" />
    </div>      
    </template>
    <div v-else>
        <h2 :style="{color: isOk ? '#222' : 'red'}">{{ statusText }}</h2>
    </div>
    <a class="btn btn-primary" href="/dragdrop">Добавить фото</a>
</template>
<script>
        ...
        async paginate (page){
            ...
            .then(result => {
                if(result.success && result.all){
                    ...
                    this.options.chunk = (result.all.last_page > this.MAX_BTN_COUNT) ? this.MAX_BTN_COUNT : result.all.last_page;
